...
async function main() {
  ...
  console.log("📋 Instructions: Press SPACE to start recording, press SPACE again to stop and process\n");
...
async function processRecording(sttPipe: any, ttsPipe: any, speaker_embeddings: string) {
  try {
    ...
    if (stats.size < 1000) { // Less than 1KB, likely empty
    ...
    if (userText.toLowerCase().includes('goodbye') || userText.toLowerCase().includes('exit')) {
      console.log("👋 Goodbye!");
      process.exit(0);
    }
